...
def small_test():
    with h5py.File('chunked3d.h5', 'w') as h5_f:
        ...
        vals = [
            k + shape[0] * j + shape[0] * shape[1] * i for i in range(shape[0])
            for j in range(shape[1]) for k in range(shape[2])
        ]
        h5_dset = h5_f.create_dataset('chunked',
                                      shape,
                                      dtype=np.int64,
                                      chunks=chunk_shape,
                                      data=vals)
...
def large_io_test():
    with h5py.File('chunked3d.h5', 'w') as h5_f:
        ...
        h5_dset = h5_f.create_dataset('large',
                                      shape,
                                      dtype=np.float64,
                                      chunks=chunk_shape,
                                      data=far_np)
