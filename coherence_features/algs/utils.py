...
def rows_range_for_applying_window(window_size, length, rows_range):
    return [(max(0,min_id-window_size), min(max_id+window_size,length)) for min_id, max_id in rows_range]
...
def moving_window(data, window, func, n_cpu=1):
    ...
    print(f'Começando execução paralela {datetime.now().strftime("%d/%m/%Y %H:%M:%S")}')
    start = timer()
