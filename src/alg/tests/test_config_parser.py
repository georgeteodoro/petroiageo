...
class TestYAMLConfig(TestCase):
    ...
    def test_can_update_base_conf_from_str(self):
        ...
        self.assertEqual(my_config.alg['feature_selection_type'], config_parser.FeatureSelection[feature_selection_type])
    ...
    def test_accepts_valid_feature_selection_mode(self):
        ...
        """
        my_config = config_parser.YAMLConfig(config_str=yaml_str_fmt.format(feature_selection_1))
        self.assertEqual(my_config.alg['feature_selection_type'], 
                         config_parser.FeatureSelection[feature_selection_1])

        feature_selection_2 = "NONE"
        my_config = config_parser.YAMLConfig(config_str=yaml_str_fmt.format(feature_selection_2))
        self.assertEqual(my_config.alg['feature_selection_type'], 
    ...
    def test_raise_invalid_save_model_types(self):
        ...
        with self.assertRaises(ValueError):
          my_config = config_parser.YAMLConfig(config_str=yaml_str_fmt.format(save_model_type))
    ...
    def test_accepts_valid_save_model_type(self):
        ...
        for model in save_model_types:
            my_config = config_parser.YAMLConfig(config_str=yaml_str_fmt.format(model))
    ...
    def test_accepts_list_of_dict_for_wells_coords(self):
        ...
        wells:
          ...
          - x: 3
            y: 3
        """
        expected_wells_coords_dicts = [{'x':1, 'y':1}, {'x':2, 'y':2}, {'x':3, 'y':3}]
        my_config = config_parser.YAMLConfig(config_str=yaml_str)
        self.assertListEqual(my_config.wells['coords'], expected_wells_coords_dicts)
    ...
    def test_raise_on_invalid_sampling_window_size(self):
      ...
      for invalid_value in invalid_values:
        with self.assertRaises(ValueError):
          my_config = config_parser.YAMLConfig(config_str=yaml_str_fmt.format(invalid_value))
    ...
    def test_raise_invalid_sampling_beta_dist_params(self):
      invalid_params = [(-1, 2), (1, -2), (-2, -2), (1, 'a'), ('a', 1), ('a', -1)]
      yaml_str_fmt="""
       wells:
         coords: [[1,1]]
