...
def main():
    ...
    ddf1['f_idx'] = ddf1.map_partitions(part_get_loc2,
                                        feature,
                                        shape,
                                        meta=(None, int))
    ...
    def update_within_part2(part_df, feature_part, feature_name):
        ...
        return np_map(part_df.f_idx.to_numpy(),
                      feature_part[feature_name].to_numpy(), min_f_id,
                      max_f_id, chunksize)

    ddf1[feature_name] = -1
    ddf1[feature_name] = ddf1.map_partitions(update_within_part2,
                                             features_ddf,
                                             feature_name,
    ...
    def get_outside_part(part, feature_name, chunksize):
        return update_given_part_np(part.f_idx.to_numpy(),
                                    part[feature_name].to_numpy(), chunksize)

    ddf1['outside_part'] = -1
    ddf1['outside_part'] = ddf1.map_partitions(get_outside_part,
                                               feature_name,
                                               chunksize,
                                               meta=(None, int))
    ddf1 = ddf1.persist()
    ...
    def update_outside_part(part, features_w, feature_name, chunksize):
        ...
        for p in part_to_fill:
            ...
            part[feature_name] = update_given_part_np(
                part.f_idx.to_numpy(), part[feature_name].to_numpy(),
                feature_part_np,
                feature_part.index.min().compute(),
                feature_part.index.max().compute(), chunksize)
    ...
    ddf1[feature_name] = ddf1.map_partitions(update_outside_part,
