...
class TrialDataSharedBase(TrialDataBase, ABC):
    ...
    def __init__(self,
        ...
        if self._is_resp_rank:
            rand_key = int(random.random()*1000)
            self._mpi_local_comm.bcast(rand_key, root=resp_rank)
        else:
            rand_key = self._mpi_local_comm.bcast(None, root=resp_rank)
            

        self._shm_lock = fasteners.InterProcessLock(
            f'/tmp/TrialDataSharedBase{rand_key}.lock')
