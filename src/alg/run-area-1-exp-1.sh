...
# module load /scratch/app/modulos/sequana/current openmpi/gnu/4.1.4_sequana
...
VENV_PATH=/petrobr/parceirosbr/petrobrasiageo/daniel.campos/envs/venv-py3.12
...
# See python3 -u main.py --help for args list 
	time vmstat 5 -S M -t -w >> $LOG_FILE_NAME & mpirun -np $(($SLURM_JOB_NUM_NODES * $N_PROCS)) --tag-output --bind-to core --map-by node python3 -u main.py --config $CONFIG_FILE --it $it --nits 1 --nsf $NSF --p-dfs --t-shd --no-abort| tee $FILENAME.log
done
if [ $ON_LOCAL -eq 1 ]
    then
    #Get out of tmp/test
