...
vmstat 5 -S M -t -w >> $LOG_FILE_NAME & mpirun -np $(($SLURM_JOB_NUM_NODES * $N_PROCS)) --map-by node --tag-output --bind-to core --oversubscribe python3 -u main.py --config $CONFIG_FILE --it $IT_START --nits $N_ITS -w $WINDOW_USED | tee $FILENAME.log
