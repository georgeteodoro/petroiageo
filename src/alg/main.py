...
RunningProcess = namedtuple('RunningProcess', 'should_update print_progress_func rank comm')
...
class FeaturesDataLoaderH5():
    def __init__(self, config:config_parser.Config):
        ...
        self.comm = config.get_param('my_process').comm
...
class PorosityCubeDataLoaderH5():
    def __init__(self, cube_path:pathlib.Path, comm):
...
class Algorithm():
    ...
    def run(self):
        ...
        with FeaturesDataLoaderH5(self.config) as feat_dl:
            print_manager("[main] Loading wells values")
            with PorosityCubeDataLoaderH5(self.config.alg['starting_porosity_cube_path'], 
                                            self.config.get_param('my_process').comm) as porosity_dl:
