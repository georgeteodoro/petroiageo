...
def treat_predicted(
    predicted: pd.DataFrame, file_path: pathlib.Path
) -> pd.DataFrame:
    ...
    if size_after_drop_dup < size_before_drop_dup:
        print(
            f"THERE WERE {size_before_drop_dup-size_after_drop_dup} DUPLICATES ON {file_path}"
        )
...
if __name__ == "__main__":
    ...
    if not second_len == first_len:
        ...
        if second_len > first_len:
            merge = second_predicted.merge(
                first_predicted, on=["x", "y", "z"], how="left", indicator=True
            )
        else:
            merge = first_predicted.merge(
                second_predicted, on=["x", "y", "z"], how="left", indicator=True
            )
    ...
    mergedDf = first_predicted.merge(
        second_predicted, on=["x", "y", "z"], how="outer", indicator=True
    )
    ...
    if len(mergedDf) > second_len:
        print(
            "WARNING! There are more points in a file than another! Look at mergedDf size!"
        )
