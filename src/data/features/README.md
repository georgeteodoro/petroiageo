...
<s>Os arquivos de *features* gerados no passo anterior estão no formato `.npy`. Devemos alterar esse formato para `.h5`. Para isso, execute seguinte comando: `seismic_data3_hdf5.py -f FEATURE_DIR -o OUTPUT_DIR --large MULT_FACTOR --config CONFIG_FILE_PATH`. Mais informações sobre cada parâmetro podem ser vistas com o comando `python3 seismic_data3_hdf5.py -h`.</s>

Os arquivos de *features* gerados no passo anterior deverão ser adicionados de paddings voltados para o uso posterior no algoritmo. Para isso, execute seguinte comando: `seismic_data3_npy.py -f FEATURE_DIR -o OUTPUT_DIR --large MULT_FACTOR --config CONFIG_FILE_PATH`. Mais informações sobre cada parâmetro podem ser vistas com o comando `python3 seismic_data3_hdf5.py -h`.
