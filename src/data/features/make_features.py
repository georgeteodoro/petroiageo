...
def calc_curvature_features(algs: set, f: str, results_folder: pathlib.Path,
    ...
    if any([feature in algs for feature in curvature_features]) or all:
        try:
            ...
            print(
                f"WARNING: Exceção capturada no cálculo dos parâmetros de curvatura!\n {e}"
            )
...
def calc_analitic_features(algs: set, f: str, results_folder: pathlib.Path,
    ...
    if any([feature in algs for feature in analitic_features]) or all:
        try:
            ...
            print(
                f"WARNING: Exceção capturada no cálculo do cubo analítico!\n {e}"
            )
            print(f"Continuando para as próximas categorias de features")
        else:
...
def calc_3d_window_features(n_cpu: int, windows_3D: list, algs: set, f: str,
    ...
    for w in windows_3D:
        ...
        if "marfurt" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_marfurt_{w}.npy",
                seismic,
                w,
                marfurt_semblance,
                n_cpu,
        ...
        if "gersz" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_gersz_{w}.npy",
                seismic,
                w,
                gersztenkorn,
                n_cpu,
        ...
        if "median" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_median_{w}.npy",
                seismic,
                w,
                np.median,
                n_cpu,
        ...
        if "mean" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_mean_{w}.npy",
                seismic,
                w,
                np.mean,
                n_cpu,
        ...
        if "min" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_min_{w}.npy",
                seismic,
                w,
                np.min,
                n_cpu,
        ...
        if "max" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_max_{w}.npy",
                seismic,
                w,
                np.max,
                n_cpu,
        ...
        if "sum" in algs or all:
            ...
            apply_and_save(
                moving_window,
                results_folder / f"{f}_sum_{w}.npy",
                seismic,
                w,
                np.sum,
                n_cpu,
...
def load_seismic_data(current_file: pathlib.Path) -> np.ndarray:
    ...
    try:
        ...
        print(
            f"ERROR: Exceção capturada ao carregar dados sísmicos de {data_file_path}!\n {e}"
        )
