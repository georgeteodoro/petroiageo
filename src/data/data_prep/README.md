...
- Áreas alvo e coordenadas (`$AREAS_COORDS_PATH`): Arquivo csv que delimita áreas de interesse no cubo sísmico baseado na concentração de poços. Criado a partir da análise da distribuição dos poços na área sísmica. Deve ter as seguintes colunas: "area, max_x, min_x, min_y, max_y". area é a identificação da área, podendo ser de qualquer tipo. As outras colunas devem ser valores inteiros não negativos com `min_x < max_x` e `min_y < max_y`
