...
def create_h5_porosity():
    with h5py.File(f"porosity_full.h5", "w") as h5_f:
        ...
        h5_dset = h5_f.create_dataset(
            "p", data_shape, dtype=data_type, chunks=chunk_shape, data=data_np
        )
    ...
    def __getitem__(self, idx):
        if isinstance(idx, numbers.Integral):
            ...
            for cur_slice in self.cur_dset_h5.iter_chunks():
                ...
                if self.train:
                    well_chunk = cur_chunk[cur_chunk["well_id"] != self.well_id]
                else:
                    well_chunk = cur_chunk[cur_chunk["well_id"] == self.well_id]
            ...
            for cur_slice in self.cur_dset_h5.iter_chunks():
                ...
                if (idx.start >= min_index) & (
                    idx.start < min_index + len(well_chunk)
                ):
                    ...
                    if idx.stop < min_index + len(well_chunk):
                        output = (
                            output
                            + well_chunk[
                                idx.start - min_index : idx.stop - min_index
                ...
                # is on a chunk ahead
                elif (idx.start < min_index) & (
                    idx.stop > min_index + len(well_chunk)
                ):
                ...
                # Otherwise, this chunk is the one with the idx stop position
                    ...
                    output = (
                        output
                        + well_chunk[0 : idx.stop - min_index][
                            self.all_features
                        ].tolist()
                    )
...
def read_only_expanded():
    ...
    real_points = fold_h5_all_clusters(
        porosity_h5, lambda d: len(d[d["real"] == 1]), 0
    )
    ...
    cur_data_type = cur_data_type + [
        (f"f{f}", np.float64) for f in range(max_features)
    ]
    ...
    cur_h5_dset = cur_h5.create_dataset(
        "c", (real_points,), dtype=cur_data_type, chunks=(cur_chunksize,)
    )
